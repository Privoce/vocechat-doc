...
VoceChat是一个完全由用户自部署使用的产品，部署成功，除了升级证书，其它所有使用均在自部署的服务器上完成，没有后门，我们不会收集任何用户的聊天记录。后期，我们会将后端开源，进一步接受监督。

#### 遥测数据说明
为了统计全球自部署节点的安装与活跃情况，我们的前端集成了一款名为 Umami 的轻量级、隐私保护型分析工具。您的隐私将得到严格保护：我们绝不使用 Cookie，也绝不记录 IP 地址、聊天记录或任何个人数据，仅收集匿名的使用次数及设备系统类型。
